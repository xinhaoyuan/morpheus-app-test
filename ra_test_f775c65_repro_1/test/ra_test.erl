...
inconsistent_state_2(Config) ->
    ...
    ?vcall(catch ra:stop_server(ServerId3)),
    ?vcall(catch enqueue(ServerId2, msg1)),
    ?vcall(catch enqueue(ServerId2, msg2)),
    timer:sleep(250),
    %% Bump term
    ?vcall(catch ra:stop_server(ServerId5)),
    ?vcall(catch ra:restart_server(ServerId5)),
    timer:sleep(250),
    ?vcall(catch ra:restart_server(ServerId3)),
    timer:sleep(250),
    %% Now tserver3 should be going to await_condition
    erlang:suspend_process(whereis(tserver3)),
    %% Bump term
    {ok, undefined, Leader2} = ra:consistent_query(ServerId2, fun (_) -> undefined end),
    ?vcall(catch ra:stop_server(Leader2)),
    ?vcall(catch ra:restart_server(Leader2)),
    timer:sleep(250),
    erlang:resume_process(whereis(tserver3)),
    timer:sleep(2000),
    ok.
