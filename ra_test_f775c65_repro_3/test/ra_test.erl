...
badkey(Config) ->
    ...
    {P, M} = spawn_monitor(fun () -> ?vcall(catch ra:remove_member(ServerId5, ServerId1)) end),
    timer:sleep(250),
    erlang:suspend_process(whereis(tserver4)),
    erlang:suspend_process(whereis(tserver5)),
    ra:restart_server(ServerId1),
    ra:restart_server(ServerId2),
    ra:restart_server(ServerId3),
    timer:sleep(250),
    ?vcall(ra:consistent_query(ServerId1, fun (_) -> undefined end)),
    erlang:resume_process(whereis(tserver4)),
    erlang:resume_process(whereis(tserver5)),
    ?vcall(enqueue(ServerId1, msg2)),
